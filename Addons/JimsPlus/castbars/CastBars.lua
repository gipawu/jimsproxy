if _G.WOW_PROJECT_ID ~= _G.WOW_PROJECT_CLASSIC then
    return print("|cFFFF0000[JimsPlus] CastBars module requires Classic Era client.|r") -- luacheck: ignore
end
...
local addon = CreateFrame("Frame", "JimsPlusCastbars")
...
function addon:PLAYER_LOGIN()
    JimsPlusCastbarsDB = JimsPlusCastbarsDB or {}

    -- Delete some old invalid settings
    if JimsPlusCastbarsDB.version and tonumber(JimsPlusCastbarsDB.version) <= 19 then
        if JimsPlusCastbarsDB.party then
            JimsPlusCastbarsDB.party.position = nil
        end
        JimsPlusCastbarsDB.player = nil
        JimsPlusCastbarsDB.npcCastUninterruptibleCache = {}
    end

    if JimsPlusCastbarsDB.npcCastUninterruptibleCache then
        JimsPlusCastbarsDB.npcCastUninterruptibleCache["11830"..GetSpellInfo(6063)] = nil
        JimsPlusCastbarsDB.npcCastUninterruptibleCache["11359"..GetSpellInfo(22678)] = nil
    end

    -- Copy any settings from defaults if they don't exist in current profile
    if JimsPlusCastbarsCharDB and JimsPlusCastbarsCharDB.usePerCharacterSettings then
        self.db = CopyDefaults(namespace.defaultConfig, JimsPlusCastbarsCharDB)
    else
        self.db = CopyDefaults(namespace.defaultConfig, JimsPlusCastbarsDB)
    end
    ...
    if not IsAddOnLoaded("ClassicCastbars_Options") and not IsAddOnLoaded("JimsPlus_Options") then
...
local channeledSpellsByName = namespace.channeledSpellsByName
...
function addon:COMBAT_LOG_EVENT_UNFILTERED()
    local _, eventType, _, srcGUID, srcName, srcFlags, _, dstGUID, _, dstFlags, _, cleuSpellId, spellName, _, missType = CombatLogGetCurrentEventInfo()
    ...
    elseif eventType == "SPELL_CAST_SUCCESS" then
        local channelCast = channeledSpells[cleuSpellId] or channeledSpellsByName[spellName]
        local spellID = castedSpells[spellName]
        ...
        -- Duration comes from our spell-ID-keyed table (exact per-rank).
        if channelCast then
            local cast = activeTimers[srcGUID]
            ...
            -- Use CLEU spellId when available, fall back to castedSpells lookup (CLEU reports id=0 for non-self through proxy)
            local resolvedId = (cleuSpellId and cleuSpellId > 0) and cleuSpellId or spellID or 0
            local icon = GetSpellTexture(resolvedId) or GetSpellTexture(spellName)
            return self:StoreCast(srcGUID, spellName, resolvedId, icon, channelCast, isSrcPlayer, true)
        else
    ...
    elseif eventType == "SPELL_AURA_REMOVED" then
        ...
        if srcGUID == dstGUID and (channeledSpells[cleuSpellId] or channeledSpellsByName[spellName]) then
            return self:DeleteCast(srcGUID, nil, nil, true)
        end
