...
public static class LanguageScrambler
{
    ...
    public static bool CanUnderstand(Race race, Class playerClass, uint language)
    {
        ...
        if (language == (uint)Language.Demonic && playerClass == Class.Warlock)
            return true;

        // Pre-login / unknown player: don't scramble.
        if (race == Race.None)
            return true;

        return race switch
        {
            Race.Human    => language == (uint)Language.Common,
            Race.Dwarf    => language == (uint)Language.Common  || language == (uint)Language.Dwarvish,
            Race.NightElf => language == (uint)Language.Common  || language == (uint)Language.Darnassian,
            Race.Gnome    => language == (uint)Language.Common  || language == (uint)Language.Gnomish,
            Race.Draenei  => language == (uint)Language.Common  || language == (uint)Language.Draenei,
            Race.Worgen   => language == (uint)Language.Common  || language == (uint)Language.Worgen,

            Race.Orc      => language == (uint)Language.Orcish,
            Race.Undead   => language == (uint)Language.Orcish  || language == (uint)Language.Gutterspeak,
            Race.Tauren   => language == (uint)Language.Orcish  || language == (uint)Language.Taurahe,
            Race.Troll    => language == (uint)Language.Orcish  || language == (uint)Language.Troll,
            Race.BloodElf => language == (uint)Language.Orcish  || language == (uint)Language.Thalassian,
            Race.Goblin   => language == (uint)Language.Orcish  || language == (uint)Language.Goblin,
